...
function DB:exec(sql)
    ...
    if not db then
        require("orange.utils.logutils").log(ngx.ERR, "failed to instantiate mysql: ", err)
        return
    end
    ...
    if not ok then
        require("orange.utils.logutils").log(ngx.ERR, "failed to connect: ", err, ": ", errno, " ", sqlstate)
        return
    end

    require("orange.utils.logutils").log(ngx.INFO, "connected to mysql, reused_times:", db:get_reused_times(), " sql:", sql)

    db:query("SET NAMES utf8")
    local res, err, errno, sqlstate = db:query(sql)
    if not res or err then
        require("orange.utils.logutils").log(ngx.ERR, "sql: ", sql, " bad result: ", err, ": ", errno, ": ", sqlstate, ".")
    end

    local ok, err = db:set_keepalive(conf.pool_config.max_idle_timeout, conf.pool_config.pool_size)
    if not ok then
        require("orange.utils.logutils").log(ngx.ERR, "failed to set keepalive: ", err)
    end
