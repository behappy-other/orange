...
local function send_result(res, format, value)
    if format == "json" then
        xpcall(function()
            ...
            require("orange.utils.logutils").log(ngx.ERR, "decode as json format error: ", trace)
        end)
...
API:get("/kvstore/get", function(store)
    return function(req, res, next)
        ...
        require("orange.utils.logutils").log(ngx.INFO, dict, " ", key, " ", format, " v:", value)
...
API:post("/kvstore/set", function(store)
    return function(req, res, next)
        ...
        if log then
            require("orange.utils.logutils").log(ngx.INFO, string_format("kvstore-set, dict: %s, key: %s, value: [[%s]], success: %s", dict, key, value, success))
        end
