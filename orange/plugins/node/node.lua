...
function _M.init(config)
    require("orange.utils.logutils").log(ngx.ERR, "node init")
end
    ...
    for _, plugin in pairs(plugins) do
        ...
        if plugin ~= 'stat' and plugin ~= 'node' then
            ...
            if not resp or err then
                require("orange.utils.logutils").log(ngx.ERR, plugin .. " sync err", err)
                sync_result[plugin] = false
            else
                sync_result[plugin] = tonumber(resp.status) == 200
                require("orange.utils.logutils").log(ngx.ERR, "status" .. resp.status, sync_result[plugin])
            end
...
function _M.sync(plugins, store)
    ...
    if not result then
        require("orange.utils.logutils").log(ngx.ERR, "SYNC", err)
    end
