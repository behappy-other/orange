            ...
            if pass then
                ...
                if handle.perform == 'allow' then
                    if handle.log == true then
                        require("orange.utils.logutils").log(ngx.INFO, "[WAF-Pass-Rule] ", rule.name, " uri:", ngx_var_uri)
                    end
                else
                    if handle.log == true then
                        require("orange.utils.logutils").log(ngx.INFO, "[WAF-Forbidden-Rule] ", rule.name, " uri:", ngx_var_uri)
                    end
...
function WAFHandler:access(conf)
    ...
    for i, sid in ipairs(ordered_selectors) do
        require("orange.utils.logutils").log(ngx.INFO, "==[WAF][PASS THROUGH SELECTOR:", sid, "]")
        ...
        if selector and selector.enable == true then
            ...
            if selector_pass then
                if selector.handle and selector.handle.log == true then
                    require("orange.utils.logutils").log(ngx.INFO, "[WAF][PASS-SELECTOR:", sid, "] ", ngx_var_uri)
                end
                ...
                if selector.handle and selector.handle.log == true then
                    require("orange.utils.logutils").log(ngx.INFO, "[WAF][NOT-PASS-SELECTOR:", sid, "] ", ngx_var_uri)
                end
            end
        end
    end
