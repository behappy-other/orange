...
local function is_authorized(signature_name, secretKey,extractor)
    ...
    local check_sig = function(extractions,secretKey)
        ...
        local md5 = md5:new()
        if not md5 then
            require("orange.utils.logutils").log(ngx.ERR,'server error exec md5:new faild')
            return false
        end
        ...
        for _, v in ipairs(param) do
            if req_val[v] then
                ...
                if not ok then
                    require("orange.utils.logutils").log(ngx.ERR,'server error exec md5:update faild')
                    return false
                end
            end
        end
            ...
            if pass then
                if handle.credentials then
                    if handle.log == true then
                        require("orange.utils.logutils").log(ngx.INFO, "[SignatureAuth-Pass-Rule] ", rule.name, " uri:", ngx_var_uri)
                    end
                    ...
                    if handle.log == true then
                        require("orange.utils.logutils").log(ngx.INFO, "[SignatureAuth-Forbidden-Rule] ", rule.name, " uri:", ngx_var_uri)
                    end
                    ngx.exit(tonumber(handle.code) or 401)
                    return true
                end
...
function SignatureAuthHandler:access(conf)
    ...
    for i, sid in ipairs(ordered_selectors) do
        require("orange.utils.logutils").log(ngx.INFO, "==[SignatureAuth][PASS THROUGH SELECTOR:", sid, "]")
        ...
        if selector and selector.enable == true then
            ...
            if selector_pass then
                if selector.handle and selector.handle.log == true then
                    require("orange.utils.logutils").log(ngx.INFO, "[SignatureAuth][PASS-SELECTOR:", sid, "] ", ngx_var_uri)
                end
                ...
                if selector.handle and selector.handle.log == true then
                    require("orange.utils.logutils").log(ngx.INFO, "[SignatureAuth][NOT-PASS-SELECTOR:", sid, "] ", ngx_var_uri)
                end
            end
        end
    end
