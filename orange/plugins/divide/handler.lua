        ...
        if rule.enable == true then
            ...
            if pass then
                if rule.log == true then
                    require("orange.utils.logutils").log(ngx.INFO, "[Divide-Match-Rule] ", rule.name, " host:", ngx_var_host, " uri:", ngx_var_uri)
                end
                ...
                if rule.upstream_url then
                    ...
                    require("orange.utils.logutils").log(ngx.INFO, "[Divide-Match-Rule:upstream] ", rule.name, " extractor_type:", extractor_type,
                        " upstream_host:", ngx_var.upstream_host, " upstream_url:", ngx_var.upstream_url)
                else
                    require("orange.utils.logutils").log(ngx.INFO, "[Divide-Match-Rule:error] no upstream host or url. ", rule.name, " host:", ngx_var_host, " uri:", ngx_var_uri)
                end

                return true
            else
                if rule.log == true then
                    require("orange.utils.logutils").log(ngx.INFO, "[Divide-NotMatch-Rule] ", rule.name, " host:", ngx_var_host, " uri:", ngx_var_uri)
                end
            end
        end
    end
...
function DivideHandler:access(conf)
    ...
    for i, sid in ipairs(ordered_selectors) do
        require("orange.utils.logutils").log(ngx.INFO, "==[Divide][PASS THROUGH SELECTOR:", sid, "]")
        ...
        if selector and selector.enable == true then
            ...
            if selector_pass then
                if selector.handle and selector.handle.log == true then
                    require("orange.utils.logutils").log(ngx.INFO, "[Divide][PASS-SELECTOR:", sid, "] ", ngx_var_uri)
                end
                ...
                if selector.handle and selector.handle.log == true then
                    require("orange.utils.logutils").log(ngx.INFO, "[Divide][NOT-PASS-SELECTOR:", sid, "] ", ngx_var_uri)
                end
            end
        end
    end
