...
local function do_filter_no_blocked(handle, rule, real_value, limit_type, limit_key, remote_addr, current_stat, blocked_num, ngx_var_uri)
    if current_stat >= handle.count then
        if handle.log == true then
            require("orange.utils.logutils").log(ngx.INFO, plugin_config.message_forbidden, rule.name, " uri:", ngx_var_uri, " limit:", handle.count, " reached:", current_stat, " remaining:", 0)
        end
        ...
        -- if handle.log == true then
        --     require("orange.utils.logutils").log(ngx.INFO, "[RateLimiting-Rule] ", rule.name, " uri:", ngx_var_uri, " limit:", handle.count, " reached:", current_stat + 1)
        -- end
    end
    return false
end
...
local function do_filter_has_blocked(handle, rule, real_value, limit_type, limit_key, remote_addr, current_stat, blocked_num, ngx_var_uri)
    ...
    require("orange.utils.logutils").log(ngx.ERR,"property_rate_limiting - block_key：",block_key)
    --判断是否处于封禁状态
    local is_blocked = get_stat_by_key(block_key)
    require("orange.utils.logutils").log(ngx.ERR,"property_rate_limiting - is_blocked：", is_blocked)
    ...
    require("orange.utils.logutils").log(ngx.ERR,"property_rate_limiting - before_handle_count：",before_handle_count)

    --如果控制台重新设置了限制次数，则加以判断
    if is_blocked and handle.count <= before_handle_count then
        if handle.log == true then
            require("orange.utils.logutils").log(ngx.ERR, plugin_config.message_forbidden, " remote_addr：", remote_addr, ' rule_name：', rule.name, " uri：", ngx_var_uri, " limit：", handle.count, " reached:", current_stat, " remaining：", 0)
        end
        ngx.header[plugin_config.plug_reponse_header_prefix ..limit_type] = 0
        ngx.exit(403)
        return true
    ...
    if current_stat >= handle.count then
        ...
        -- only for test, comment it in production
        -- if handle.log == true then
        --     require("orange.utils.logutils").log(ngx.ERR, "[RateLimiting-Rule] ", rule.name, " uri:", ngx_var_uri, " limit:", handle.count, " reached:", current_stat + 1)
        -- end
    end
    return false
end
...
local function filter_rules(sid, plugin, ngx_var_uri)
    ...
    for i, rule in ipairs(rules) do
        if rule.enable == true then
            require("orange.utils.logutils").log(ngx.ERR, "property_rate_limiting - rule.extractor: ", sp_utils.tableToStr(rule.extractor))
            -- 变量取值
            local real_value = table_concat( extractor_util.extract_variables(rule.extractor),"#")
            require("orange.utils.logutils").log(ngx.ERR, "property_rate_limiting - real_value: ", real_value)
            ...
            if pass then
                ...
                require("orange.utils.logutils").log(ngx.ERR, "property_rate_limiting - limit_type: ", limit_type)
                ...
                if limit_type then
                    ...
                    require("orange.utils.logutils").log(ngx.ERR,"property_rate_limiting - time_key：",time_key)
                    local limit_key = rule.id .. "#" .. time_key .. "#" .. real_value
                    require("orange.utils.logutils").log(ngx.ERR,"property_rate_limiting - limit_key：",limit_key)
                    --得到当前缓存中limit_key的数量 - 对应该time_key下已访问的数量
                    local current_stat = get_stat_by_key(limit_key) or 0
                    require("orange.utils.logutils").log(ngx.ERR,"property_rate_limiting - current_stat：",current_stat,", limit_type：",limit_type)

                    --如果blocked不为空，则需要进行封禁403。反之429
                    require("orange.utils.logutils").log(ngx.ERR, "property_rate_limiting - blocked_num: ", blocked_num)
...
function PropertyRateLimitingHandler:access(conf)
    ...
    for i, sid in ipairs(ordered_selectors) do
        require("orange.utils.logutils").log(ngx.ERR, "==[",plugin_config.name_for_log,"][PASS THROUGH SELECTOR:", sid, "]")
        ...
        if selector and selector.enable == true then
            ...
            if selector_pass then
                if selector.handle and selector.handle.log == true then
                    require("orange.utils.logutils").log(ngx.ERR, "[",plugin_config.name_for_log,"][PASS-SELECTOR:", sid, "] ", ngx_var_uri)
                end
                ...
                if selector.handle and selector.handle.log == true then
                    require("orange.utils.logutils").log(ngx.ERR, "[",plugin_config.name_for_log,"][NOT-PASS-SELECTOR:", sid, "] ", ngx_var_uri)
                end
            end
        end
    end
