...
function _M.get_rules_of_selector(plugin, store, rule_ids)
    ...
    if err then
        require("orange.utils.logutils").log(ngx.ERR, "error to get rules of selector, err:", err)
        return {}
    end
...
function _M.delete_rules_of_selector(plugin, store, rule_ids)
    ...
    if delete_result then
        ...
        require("orange.utils.logutils").log(ngx.ERR, "delete rules of selector err, ", rule_ids)
        return false
    end
end
...
function _M.delete_selector(plugin, store, selector_id)
    ...
    if delete_result then
        ...
        require("orange.utils.logutils").log(ngx.ERR, "delete selector err, ", selector_id)
        return false
    end
end
...
function _M.get_meta(plugin, store)
    ...
    if not err and meta and type(meta) == "table" and #meta > 0 then
        ...
        require("orange.utils.logutils").log(ngx.ERR, "[FATAL ERROR]meta not found while it must exist.")
        return nil
    end
end
...
function _M.update_meta(plugin, store, meta)
    ...
    if not meta_json_str then
        require("orange.utils.logutils").log(ngx.ERR, "encode error: meta to save is not json format.")
        return false
    end
...
function _M.update_selector(plugin, store, selector)
    ...
    if not selector_json_str then
        require("orange.utils.logutils").log(ngx.ERR, "encode error: selector to save is not json format.")
        return false
    end
...
function _M.update_local_meta(plugin, store)
    ...
    if err then
        require("orange.utils.logutils").log(ngx.ERR, "error to find meta from storage when updating local meta, err:", err)
        return false
    end
    ...
    if meta and type(meta) == "table" and #meta > 0 then
        ...
        if err or not success then
            require("orange.utils.logutils").log(ngx.ERR, "update local plugin's meta error, err:", err)
            return false
        end
    else
        require("orange.utils.logutils").log(ngx.ERR, "can not find meta from storage when updating local meta")
    end
...
function _M.update_local_selectors(plugin, store)
    ...
    if err then
        require("orange.utils.logutils").log(ngx.ERR, "error to find selectors from storage when updating local selectors, err:", err)
        return false
    end
    ...
    if selectors and type(selectors) == "table" then
        ...
        if err or not success then
            require("orange.utils.logutils").log(ngx.ERR, "update local plugin's selectors error, err:", err)
            return false
        end
    else
        require("orange.utils.logutils").log(ngx.ERR, "the size of selectors from storage is 0 when updating local selectors")
...
function _M.update_local_selector_rules(plugin, store, selector_id)
    if not selector_id then
        require("orange.utils.logutils").log(ngx.ERR, "error to find selector from storage when updating local selector rules, selector_id is nil")
        return false
    end

    local selector = _M.get_selector(plugin, store, selector_id)
    if not selector or not selector.value then
        require("orange.utils.logutils").log(ngx.ERR, "error to find selector from storage when updating local selector rules, selector_id:", selector_id)
        return false
    end
    ...
    if err or not success then
        require("orange.utils.logutils").log(ngx.ERR, "update local rules of selector error, err:", err)
        return false
    end
...
function _M.init_rules_of_selector(plugin, store, selector_id)
    if not selector_id then
        require("orange.utils.logutils").log(ngx.ERR, "error: selector_id is nil")
        return false
    end

    local selector = _M.get_selector(plugin, store, selector_id)
    if not selector or not selector.value then
        require("orange.utils.logutils").log(ngx.ERR, "error to find selector from storage when initializing plugin[" .. plugin .. "] local selector rules, selector_id:", selector_id)
        return false
    end
    ...
    local success, err, forcible = orange_db.set_json(plugin .. ".selector." .. selector_id .. ".rules", rules)
    if err or not success then
        require("orange.utils.logutils").log(ngx.ERR, "init plugin[" .. plugin .. "] local rules of selector error, err:", err)
        return false
    end
...
function _M.init_enable_of_plugin(plugin, store)
    ...
    if err then
        require("orange.utils.logutils").log(ngx.ERR, "Load `enable` of plugin[" .. plugin .. "], error: ", err)
        return false
    end
...
function _M.init_meta_of_plugin(plugin, store)
    ...
    if err then
        require("orange.utils.logutils").log(ngx.ERR, "error to find meta from storage when initializing plugin[" .. plugin .. "] local meta, err:", err)
        return false
    end
    ...
    if meta and type(meta) == "table" and #meta > 0 then
        local success, err, forcible = orange_db.set(plugin .. ".meta", meta[1].value or '{}')
        if err or not success then
            require("orange.utils.logutils").log(ngx.ERR, "init local plugin[" .. plugin .. "] meta error, err:", err)
            return false
        end
    else
        require("orange.utils.logutils").log(ngx.ERR, "can not find meta from storage when initializing plugin[" .. plugin .. "] local meta")
    end
...
function _M.init_selectors_of_plugin(plugin, store)
    ...
    if err then
        require("orange.utils.logutils").log(ngx.ERR, "error to find selectors from storage when initializing plugin[" .. plugin .. "], err:", err)
        return false
    end
    ...
    if selectors and type(selectors) == "table" then
        ...
        local success, err, forcible = orange_db.set_json(plugin .. ".selectors", to_update_selectors)
        if err or not success then
            require("orange.utils.logutils").log(ngx.ERR, "init local plugin[" .. plugin .. "] selectors error, err:", err)
            return false
        end
    else
        require("orange.utils.logutils").log(ngx.ERR, "the size of selectors from storage is 0 when initializing plugin[" .. plugin .. "] local selectors")
        local success, err, forcible = orange_db.set_json(plugin .. ".selectors", {})
        if err or not success then
            require("orange.utils.logutils").log(ngx.ERR, "init local plugin[" .. plugin .. "] selectors error, err:", err)
            return false
...
function _M.compose_plugin_data(store, plugin)
    ...
    ok = xpcall(function()
        ...
        if err then
            require("orange.utils.logutils").log(ngx.ERR, "error to find meta from storage when fetching data of plugin[" .. plugin .. "], err:", err)
            return false
        end
        ...
        if meta and type(meta) == "table" and #meta > 0 then
            ...
            require("orange.utils.logutils").log(ngx.ERR, "can not find meta from storage when fetching data of plugin[" .. plugin .. "]")
            return false
        end
        ...
        if err then
            require("orange.utils.logutils").log(ngx.ERR, "error to find selectors from storage when fetching data of plugin[" .. plugin .. "], err:", err)
            return false
        end
        ...
        if selectors and type(selectors) == "table" then
            for _, s in ipairs(selectors) do
                ...
                local selector = _M.get_selector(plugin, store, selector_id)
                if not selector or not selector.value then
                    require("orange.utils.logutils").log(ngx.ERR, "error to find selector from storage when fetch plugin[" .. plugin .. "] selector rules, selector_id:", selector_id)
                    return false
                end
            ...
            require("orange.utils.logutils").log(ngx.ERR, "the size of selectors from storage is 0 when fetching data of plugin[" .. plugin .. "] selectors")
    ...
    if not ok or e then
        require("orange.utils.logutils").log(ngx.ERR, "[fetch plugin's data error], plugin:", plugin, " error:", e)
        return false
    end
...
function _M.load_data_by_mysql(store, plugin)
    ...
    ok = xpcall(function()
        local v = plugin
        if not v or v == "" then
            require("orange.utils.logutils").log(ngx.ERR, "params error, the `plugin` is nil")
            return false
        end
        ...
        elseif v == "kvstore" then
            ...
            if not init_enable then
                require("orange.utils.logutils").log(ngx.ERR, "load data of plugin[" .. v .. "] error, init_enable:", init_enable)
                return false
            else
                require("orange.utils.logutils").log(ngx.INFO, "load data of plugin[" .. v .. "] success")
            end
            ...
            if not init_enable or not init_meta or not init_selectors_and_rules then
                require("orange.utils.logutils").log(ngx.ERR, "load data of plugin[" .. v .. "] error, init_enable:", init_enable, " init_meta:", init_meta, " init_selectors_and_rules:", init_selectors_and_rules)
                return false
            else
                require("orange.utils.logutils").log(ngx.INFO, "load data of plugin[" .. v .. "] success")
            end
    ...
    if not ok or e then
        require("orange.utils.logutils").log(ngx.ERR, "[load plugin's data error], plugin:", plugin, " error:", e)
        return false
    end
