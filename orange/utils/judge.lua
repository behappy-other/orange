...
function _M.parse_conditions(expression, params)
    ...
    for i, v in ipairs(params) do
        if v == nil or type(v) ~= "boolean" then
            require("orange.utils.logutils").log(ngx.ERR, "condition value[", v, "] is nil or not a `boolean` value.")
            return false
        end
...
function _M.filter_complicated_conditions(expression, conditions, plugin_name)
    ...
    if not func or err then
        require("orange.utils.logutils").log(ngx.ERR, "failed to load script: ", condition)
        return false
    end

    pass = func()
    if pass then
        require("orange.utils.logutils").log(ngx.INFO, "[", plugin_name or "", "]filter_complicated_conditions: ", expression)
    end
