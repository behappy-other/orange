...
local function assert_condition(real, operator, expected)
    if not real then
        require("orange.utils.logutils").log(ngx.ERR, string_format("assert_condition error: %s %s %s", real, operator, expected))
        return false
    end
...
function _M.judge(condition)
    ...
    elseif condition_type == "PostParams" then
        ...
        if not post_params or err then
            require("orange.utils.logutils").log(ngx.ERR, "[Condition Judge]failed to get post args: ", err)
            return false
        end
