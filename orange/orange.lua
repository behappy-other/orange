...
local function load_node_plugins(config, store)
    require("orange.utils.logutils").log(ngx.DEBUG, "Discovering used plugins")
    ...
    for _, v in ipairs(plugins) do
        ...
        if not loaded then
            require("orange.utils.logutils").log(ngx.WARN, "The following plugin is not installed or has no handler: " .. v)
        else
            require("orange.utils.logutils").log(ngx.DEBUG, "Loading plugin: " .. v)
...
function Orange.init(options)
    ...
    if not status or err then
        require("orange.utils.logutils").log(ngx.ERR, "Startup error: " .. err)
        os.exit(1)
    end
...
function Orange.init_worker()
    ...
    if Orange.data and Orange.data.store and Orange.data.config.store == "mysql" then
            ...
            if not ok then
                require("orange.utils.logutils").log(ngx.ERR, "failed to create the timer: ", err)
                return os.exit(1)
            end
    end
